...
axios.get(`https://uptime-api.0xanupx0.repl.co/uptime?link=${UrlRepl}`)
		logger(global.fca.languages.uptime + UrlRepl, '[ ZACH - REMADE ]');
	}
	catch {
		logger(global.fca.languages.erroUptime, '[ ZACH - REMADE ]');
  }
}
...
function buildAPI(globalOptions, html, jar) {
    ...
    logger(`${global.fca.languages.loginSu}${userID}`, "[ ZACH - REMADE ]");
    ...
    if (oldFBMQTTMatch) {
        ...
        logger(`${global.fca.languages.region}${region}`, "[ ZACH - REMADE ]");
    } else {
                                            ...
                                            utils
                                                ...
                                                .then(res => {
                                                    try {
                                                        ...
                                                        clearInterval(checkVerified);
                                                        logger(global.fca.languages.okweb, "[ FCA - ZACH ]");
                                                        if (callback === prCallback) {
                                                            callback = function(err, api) {
                                                                if (err) return prReject(err);
                                                                return prResolve(api);
...
require('dotenv').config({ path: './../.env' });
                ...
                var ans = makeid(49)
                    process.env["FBKEY"] = ans;
                        fs.writeFile('./../.env', `FBKEY=${ans}`, function (err) {
                            if (err) {
                            logger(global.fca.languages.errorEnv, "[ ZACH - REMADE ]");
                    }
                else logger(global.fca.languages.env,"[ ZACH - REMADE ]")
        }); 
    }
    catch (e) {
        console.log(e);
        logger(global.fca.languages.errorPassRandom, "[ ZACH - REMADE ]");
    }
}
    ...
    if (process.env['FBKEY']) {
        try {
            ...
            if (appState.includes('[')) {
                logger(global.fca.languages.appstateDec, '[ ZACH - REMADE ]');
            } else {
                try {
                    appState = JSON.parse(appState);
                    var StateCrypt = require('./StateCrypt');
                    appState = StateCrypt.decryptState(appState, process.env['FBKEY']);
                    logger(global.fca.languages.appstate, '[ ZACH - REMADE ]');
                }
                catch (e) {
                    logger(global.fca.languages.appstateError, '[ ZACH - REMADE ]');
                }
            }
        }
        catch (e) {
    ...
    try {
        ...
        try {
            ...
            return logger(global.fca.languages.appstateError, '[ ZACH - REMADE ]')
        }
    }
      }
    try { 
    ...
    mainPromise
        .then(function() {
            logger(global.fca.languages.oklogin, "[ ZACH - REMADE ]");
